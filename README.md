...
5. Нажать в VS Code Ctrl+Shift+P, набрать "keyboard" и выбрать пункт Preferences: Open Keyboard Shortcuts (JSON), скопировать содержимое файла [keybindings.json](.vscode/keybindings.json) в открывшуюся вкладку и затем сохранить этот файл личных настроек (Ctrl+S)
6. Выбрать README.md слева в списке файлов и внимательно его прочитать (разделы **"Содержимое"** и **"Рекомендации"** ниже)
7. Переделать все под себя и have fun
